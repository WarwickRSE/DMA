...
class repeating_penta_thomas{
  ...
  // Update stored values for a new array (including size changes)
  // Will completely wipe out any previous values
  void update_array(penta_repeating const & new_arr){
...
bool verify_stored_array(penta_repeating expected){
    banded_general_penta L(len), U(len), res(len);
    ...
    for(int i = 0; i < 5; i++){
        for(int j = 0; j< res.values[i].size(); j++){
            if(std::abs(res.values[i][j] - expected.get(i, j)) > zero_thresh) err = true;
        }
    }
    assert(!err);
    if(err) res.print();
